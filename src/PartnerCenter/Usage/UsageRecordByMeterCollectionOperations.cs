...
namespace Microsoft.Store.PartnerCenter.Usage
{
    ...
    internal class UsageRecordByMeterCollectionOperations : BasePartnerComponent<Tuple<string, string>>, IUsageRecordByMeterCollection
    {
        ...
        public async Task<ResourceCollection<MeterUsageRecord>> GetAsync(CancellationToken cancellationToken = default)
        {
            return await Partner.ServiceClient.GetAsync<ResourceCollection<MeterUsageRecord>>(
                new Uri(
                    string.Format(
                        CultureInfo.InvariantCulture,
                        $"/{PartnerService.Instance.ApiVersion}/{PartnerService.Instance.Configuration.Apis.GetSubscriptionUsageRecordsByMeter.Path}",
                        Context.Item1,
                        Context.Item2),
                    UriKind.Relative),
